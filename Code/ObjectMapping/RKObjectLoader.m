...
- (id)initWithResourcePath:(NSString*)resourcePath client:(RKClient*)client mapper:(RKObjectMapper*)mapper delegate:(NSObject<RKObjectLoaderDelegate>*)delegate {
	if ((self = [self initWithURL:[client URLForResourcePath:resourcePath] delegate:delegate client:self.client])) {
...
- (void)processLoadModelsInBackground:(RKResponse *)response {
	...
	if (nil != error) {
		NSDictionary* infoDictionary = [[[NSDictionary dictionaryWithObjectsAndKeys:response, @"response", error, @"error", nil] retain] autorelease];
		...
		NSDictionary* infoDictionary = [[[NSDictionary dictionaryWithObjectsAndKeys:response, @"response", models, @"models", nil] retain] autorelease];
